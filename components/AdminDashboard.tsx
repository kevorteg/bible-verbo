                                    ...
                                    <tbody className="divide-y divide-neutral-100">
                                        {filteredUsers.map(u => (
                                            <tr key={u.id} className="hover:bg-neutral-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-wider ${u.role === 'admin' ? 'bg-purple-100 text-purple-600' :
                                                ...
                                                <td className="px-6 py-4">
                                                    <button
                                                        onClick={async () => {
                                                            if (isAdmin) {
                                                                try {
                                                                    await UserService.updateUserVerification(u.id, !u.isVerified);
                                                                    setUsers(users.map(user => user.id === u.id ? { ...user, isVerified: !u.isVerified } : user));
                                                                } catch (e) {
                                                                    alert("Error al actualizar verificación");
                                                                }
                                                            }
                                                        }}
                                                        className={`px-2 py-1 rounded text-[10px] font-bold uppercase transition-all ${u.isVerified
                                                                ? 'bg-teal-100 text-teal-600 hover:bg-teal-200'
                                                                : 'bg-neutral-100 text-neutral-400 hover:bg-neutral-200'
                                                            }`}
                                                        title={isAdmin ? "Click para cambiar estado" : ""}
                                                        disabled={!isAdmin}
                                                    >
                                                        {u.isVerified ? 'Verificado' : 'No Verif.'}
                                                    </button>
                                                </td>
                                                <td className="px-6 py-4">
